...
def run(conf):
    ...
    try:
        for _ in range(conf['num_workers']):
            server = WebServer(http_handler=HttpHandler, host=conf['host'], port=conf['port'],
                               document_root=conf['document_root'])
            p = multiprocessing.Process(target=server.serve_forever, name='worker {}'.format(_))
            processes.append(p)
            logging.basicConfig(filename=conf['file_log'], level=level,
                                format='[%(asctime)s] %(levelname).1s %(message)s', datefmt='%Y.%m.%d %H:%M:%S')
            p.start()
            logging.info('Server running on the process: '
                         'name: worker {0} pid: {1} host: {2}, port: {3}'.format(_, p.pid, conf['host'], conf['port']))
        for p in processes:
            p.join()
...
if __name__ == '__main__':
    ...
    args_parse.add_argument('-l', '--level_log', default='debug')
    args_parse.add_argument('-f', '--file_log', default=os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                                     'httpd.log'))
